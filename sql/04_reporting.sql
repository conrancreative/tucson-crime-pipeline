...
--   UAPD
-- University of Arizona PD bike THEFTS (crashes stay in mart_uapd_bike),
...
select 'UA:' || case_number as id
    ...
    -- Old Main fallback
    , coalesce(lat, 32.2313)::double precision  as lat 
    , coalesce(lon, -110.9558)::double precision as lon
    , now() as pulled_at
    , (now() at time zone 'America/Phoenix') as pulled_at_az
    , 'UAPD'::text as source
    , (lat is not null) as geocoded
